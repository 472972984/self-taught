...
public class ThirdChainController {
    ...
    @Value("${hde.url:https://10.50.1.67/hde/open/api/entrance}")
    ...
    @Value("${hde.chainCode:test-chain-code}")
    ...
    public Result hdeOrderInformPost(@RequestBody OrderInformDTO orderInformDTO) throws Exception {
        ...
        List<String> orderCodeList = orderInformDTO.getOrderCode();
        log.info("收到hde订单信息推送：订单号为：{}, 商户号为：{}", orderCodeList, orderInformDTO.getMerchantId());
        for (String orderCode : orderCodeList) {
            try {
                //调用hde服务获取订单详情
                String method = "hde.order.detail.get";
                JSONObject jsonObject = new JSONObject();
                jsonObject.set("orderCode", orderCode);
