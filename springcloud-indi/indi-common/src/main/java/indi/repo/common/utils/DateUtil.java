...
public class DateUtil {
    ...
    public static Date dateValid(String date) {
        Date d = null;
        try {
            d = new SimpleDateFormat(PATTERN_DATETIME).parse(date);
        } catch (ParseException pe) {
        }
        return d;
    }
    ...
    public static Date formatStartDay(Date date) {
        String day = dateFormat(date, "yyyy-MM-dd");
        return dateFormat(day + " 00:00:00");
    }

    public static Date formatEndDay(Date date) {
        String day = dateFormat(date, "yyyy-MM-dd");
        return dateFormat(day + " 23:59:59");
    }

    public static Date formatStartDay(String date) {
        return dateFormat(date + " 00:00:00");
    }
